...
class TestTransferOnXgmi(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        ret = cls.engine.initialize(cls.local_hostname, "P2PHANDSHAKE", "xgmi", "")
        if ret != 0:
            raise unittest.SkipTest(f"Failed to initialize XGMI transport (code {ret})")
        ...
        if ret1 != 0 or ret2 != 0:
            raise RuntimeError("Failed to register XGMI buffers")
