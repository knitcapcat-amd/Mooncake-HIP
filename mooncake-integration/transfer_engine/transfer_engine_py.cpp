...
void initMemoryAllocator(const char *protocol) {
    ...
    if (proto == "nvlink") {
        ...
        LOG(INFO) << "Selected XGMI (HIP) memory allocator";
#else
        allocateMemory = malloc;
        freeMemory = free;
        LOG(ERROR) << "Protocol 'xgmi' requires -DUSE_HIP=ON";
#endif
    } else if (proto == "hip") {
        allocateMemory = malloc;
        freeMemory = free;
        LOG(ERROR) << "Protocol 'hip' is not exposed in the Python API. "
                   << "Use 'xgmi' instead.";
    } else if (proto == "nvlink_intra") {
#ifdef USE_INTRA_NVLINK
        allocateMemory = [](size_t s) -> void * {
...
int TransferEnginePy::initializeExt(const char *local_hostname,
    ...
    const bool use_xgmi = (proto == "xgmi");
    const char *transport_name = use_xgmi ? "hip" : proto.c_str();
    ...
    bool manual_transport =
        use_xgmi || proto == "nvlink" || proto == "nvlink_intra";
    ...
    if (use_efa) {
        ...
        auto *transport = engine_->installTransport(transport_name, nullptr);
        if (!transport) {
            LOG(ERROR) << "Failed to install transport: " << transport_name;
            return -1;
        }
    } else {
