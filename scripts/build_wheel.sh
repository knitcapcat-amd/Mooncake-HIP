...
# Copy engine extension to mooncake directory (preserve ABI-tagged filename)
cp build/mooncake-integration/engine.*.so mooncake-wheel/mooncake/

# Copy libasio.so to mooncake directory (runtime dependency of engine extension)
cp build/mooncake-asio/libasio.so mooncake-wheel/mooncake/libasio.so

# Copy store extension to mooncake directory (preserve ABI-tagged filename)
if [ -f build/mooncake-integration/store.*.so ]; then
    echo "Copying store extension..."
    cp build/mooncake-integration/store.*.so mooncake-wheel/mooncake/
...
if [ "$PYTHON_VERSION" = "3.8" ]; then
    ...
    for pattern in "${PATTERNS[@]}"; do
        for libpath in /opt/rocm* /usr/local/cuda* /usr/local/cuda-12.8/lib* /usr/lib* /usr/local/lib* /lib*; do
            if [ -d "$libpath" ]; then
    ...
    EXCLUDE_OPTS="${EXCLUDE_OPTS} --exclude libcuda.so.1 --exclude libamdhip64.so.7 --exclude libhsa-runtime64.so.1 --exclude librocprofiler-register.so.0 "
